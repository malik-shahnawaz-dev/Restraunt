...
const FILTERS = ['All', 'Food', 'Interior', 'Chef', 'Kitchen', 'Atmosphere', 'Drinks']

export default function GalleryPage() {
  const [filter, setFilter] = useState('All')
  const [lightbox, setLightbox] = useState(null)

  const visible = GALLERY.filter((g) => filter === 'All' || g.tag === filter)
  ...
  return (
    <>
      ...
      <Modal open={Boolean(lightbox)} onClose={() => setLightbox(null)} size="lg" title={lightbox?.alt || 'Gallery image'}>
        {lightbox && (
          <figure className="p-3 sm:p-5">
            <img src={lightbox.src} alt={lightbox.alt} className="max-h-[72vh] w-full rounded-xl object-contain" />
            <figcaption className="mt-4 flex items-center justify-between px-1 pb-1">
              <span className="text-[14px] font-medium text-ink">{lightbox.alt}</span>
